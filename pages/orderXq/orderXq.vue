<template>
	<view class="indexbox">
	  <!-- 信息列表 -->
	    <view class="contbox bg1 radiu15">
	      <view class="msgLis" v-for="item in indexForm">
	        <view class="left font26 color2">{{item.title}}</view>
	        <view class="right font28 color2">{{item.msginfor}}</view>
	      </view>
	      <button class="finalBtn pubbj color5 center font26 radiu40" bindtap="tapName" @click="Router.redirectTo({route:{path:'/pages/toPay/toPay'}})">结算</button>
	    </view>
	    <button class="pubbj bigBtnB color5 font15" type="submit" @click="Router.redirectTo({route:{path:'/pages/toPay/toPay'}})">去结算</button>
	</view>
</template>
